...
class mitsuba_integrator(declarative_property_group):
	...
	properties = [
		...
		{
			'type': 'int',
			'attr': 'luminaireSamples',
			'name': 'Luminaire Samples',
			'description': 'Number of samples to take using the luminaire sampling technique',
			'save_in_preset': True,
			'min': 1,
			'max': 512,
			'default': 1
		...
		},
		{
			...
			'attr': 'directSamples',
			'name': 'Direct Samples',
			'description': 'Number on luminaire samples for direct illuminaiton',
			'save_in_preset': True,
			'min': 2,
			'max': 100,
			'default': 16
	...
	def get_params(self):
		params = ParamSet()
		if self.type == 'direct':
			params.add_integer('luminaireSamples', self.luminaireSamples)
			params.add_integer('bsdfSamples', self.bsdfSamples)
		elif self.type == 'path' or self.type == 'volpath' or self.type == 'ptracer' or self.type == 'volpath_simple':
