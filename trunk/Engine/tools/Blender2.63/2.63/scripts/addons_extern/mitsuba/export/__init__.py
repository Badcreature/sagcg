...
class MtsExporter:
   ...
   def exportMaterial(self, mat):
      ...
      if mmat.twosided and mmat.type in ['diffuse', 'roughdiffuse', 'phong', 'irawan', 'mask', 'dipolebrdf', 'bump',  'sssbrdf', 'ward', 
   ...
   def exportCameraSettings(self, scene, camera):
      ...
      if mcam.useDOF == True:
         ...
         self.parameter('float', 'focusDepth', {'value' : str(mcam.focusDepth)})
         self.closeElement()
      self.openElement('film', {'id' : 'film','type':str(mcam.film)})
      if str(mcam.film) == 'pngfilm':
