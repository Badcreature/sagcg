...
def MAhs():
    if 'Ahsap' not in bpy.data.materials:
        mtl=bpy.data.materials.new('Ahsap')
        mtl.diffuse_color     = (0.3,0.18,0.12)
        mtl.diffuse_shader    = 'LAMBERT' 
        mtl.diffuse_intensity = 1.0 
    else:
        mtl=bpy.data.materials['Cam']
    return mtl
def MPVC():
...
def MCam():
    ...
    if 'Cam' not in bpy.data.materials:
        mtl = bpy.data.materials.new('Cam')
    ...
    return mtl
def MMer():
    if 'Mermer' not in bpy.data.materials:
        mtl=bpy.data.materials.new('Mermer')
        mtl.diffuse_color     = (0.2,0.1,0.1)
        mtl.diffuse_shader    = 'LAMBERT' 
        mtl.diffuse_intensity = 1.0 
    else:
        mtl=bpy.data.materials['Mermer']
    return mtl
    ...
    for x in range(0,mx*2):
        for z in range(0,my*2):
            if x%2==1 and z%2==1:
                ...
                n=len(fc)
                cam.append(n-1);cam.append(n-2);cam.append(n-3)
                cam.append(n-4);cam.append(n-5);cam.append(n-6)
    #Mermer
    if self.mr==True:
        mrh=-self.mr1/100
        mrg= self.mr2/100
...
#OBJE -----------------------------------------------------------
    ...
    mesh.materials.append(MPVC());mesh.materials.append(MCam());mesh.materials.append(MMer())
    for i in cam:
...
class OBJECT_OT_add_object(bpy.types.Operator):
    ...
    prs = EnumProperty(items = (('1',"PENCERE 250X200",""),
                                ('2',"PENCERE 200X200",""),
                                ('3',"PENCERE 180X200",""),
                                ('4',"PENCERE 180X160",""),
                                ('5',"PENCERE 160X160",""),
                                ...
                                name="",description="")
    son=prs
    gen=IntProperty(name='Yatay',    min=1,max= 8, default= 3, description='Yatay Bolme')
    yuk=IntProperty(name='Dikey',    min=1,max= 5, default= 1, description='Dikey Bolme')
    kl1=IntProperty(name='Cerceve',  min=2,max=50, default= 5, description='Cerceve Kalinligi')
    kl2=IntProperty(name='Aralik',   min=2,max=50, default= 5, description='Aralik Kalinligi')
    fk =IntProperty(name='Fitil',    min=1,max=20, default= 2, description='Fitil Kalinligi')

    mr=BoolProperty(name='Mermer', default=True,description='Mermer')
    mr1=IntProperty(min=1, max=20, default= 4,  description='Yukseklik')
    mr2=IntProperty(min=0, max=20, default= 4,  description='Genislik')
    mr3=IntProperty(min=1, max=50, default=20,  description='Duvar Kalinligi')
    mr4=IntProperty(min=0, max=50, default= 0,  description='Sove Kalinligi')

    gnx0=IntProperty(min=1,max=300,default= 60,description='Genislik')
    gnx1=IntProperty(min=1,max=300,default=110,description='Genislik')
    gnx2=IntProperty(min=1,max=300,default= 60,description='Genislik')
    gnx3=IntProperty(min=1,max=300,default= 60,description='Genislik')
    gnx4=IntProperty(min=1,max=300,default= 60,description='Genislik')
    gnx5=IntProperty(min=1,max=300,default= 60,description='Genislik')
    gnx6=IntProperty(min=1,max=300,default= 60,description='Genislik')
    gnx7=IntProperty(min=1,max=300,default= 60,description='Genislik')

    gny0=IntProperty(min=1,max=300,default=190,description='Yukseklik')
    gny1=IntProperty(min=1,max=300,default= 45,description='Yukseklik')
    gny2=IntProperty(min=1,max=300,default= 45,description='Yukseklik')
    gny3=IntProperty(min=1,max=300,default= 45,description='Yukseklik')
    gny4=IntProperty(min=1,max=300,default= 45,description='Yukseklik')
