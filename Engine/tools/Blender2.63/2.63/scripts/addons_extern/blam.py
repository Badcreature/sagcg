...
class ProjectBackgroundImageOntoMeshOperator(bpy.types.Operator):
    ...
    def addUVsProjectedFromView(self, mesh):
        ...
        for meshFace, uvFace in zip(mesh.data.faces, mesh.data.uv_textures[0].data):
...
class Reconstruct3DMeshOperator(bpy.types.Operator):
    ...
    def createMesh(self, inputMesh, computedCoordsByFace, quads):
        ...
        #face is set to 1)
        k1 = 1
        firstFace = inputMesh.data.faces[0]
        numFaces = len(inputMesh.data.faces)
        faces = [f for f in inputMesh.data.faces]
        matrixRows = []
...
class CameraCalibrationOperator(bpy.types.Operator):
    ...
    def computeFocalLength(self, Fu, Fv, P):
        ...
        #if fSq < 0: TODO: report this case
    ...
    def computeCameraRotationMatrix(self, Fu, Fv, f):
    ...
    def execute(self, context):
        ...
        principalPoint = activeSpace.clip.tracking.camera.principal
        principalPoint = [0.5, 0.5]#TODO: implement this [principalPoint[0] / imageWidth, principalPoint[1] / imageHeight]
        P = self.relImgCoords2ImgPlaneCoords(principalPoint, imageWidth, imageHeight)
        ...
        if singleVp:
            ...
            '''
            compute focal length and apply it to the active camera
            '''
            ...
            #compute the relative focal length
        ...
        M = self.computeCameraRotationMatrix(Fu, Fv, f)
...
def initSceneProps():
    ...
    desc = "Automatically set the current movie clip as the camera background image when performing camera calibration."
